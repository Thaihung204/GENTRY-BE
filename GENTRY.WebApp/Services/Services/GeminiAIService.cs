...
namespace GENTRY.WebApp.Services.Services
{
    public class GeminiAIService : BaseService, IGeminiAIService
    {
            ...
            try
            {
                ...
                try
                {
                    outfitId = await SaveGeneratedOutfitAsync(
                        selectedItems,
                        userId,
                        $"GENTRY AI Generated: {request.UserMessage}"
                    );
                }
                catch (Exception ex)
                {
                ...
                // 8️⃣ Lưu chat history vào database
                var chatHistoryId = Guid.Empty;
                try
                {
                    chatHistoryId = await SaveChatHistoryAsync(request, aiResponse, userId, outfitId);
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Lỗi khi lưu chat history vào DB");
                }
        ...
        private async Task<Guid> SaveChatHistoryAsync(OutfitAIRequestDto request, string aiResponse, Guid userId, Guid? outfitId = null)
        {
            try
            {
                var chatHistory = new ChatHistory
                {
                    ...
                    GeneratedOutfitId = outfitId,
                    ChatType = "OutfitRecommendation",
                    IsActive = true,
                    CreatedDate = DateTime.UtcNow
                };

                await Repo.CreateAsync(chatHistory);
                await Repo.SaveAsync();
